...
namespace KarstNSim {
	...
	*/
	...
	public:
		...
		*/
		void set_sinks(std::vector<Vector3>* sinks, std::vector<int> propsinksindex, std::vector<int> propsinksorder);
		...
		*/
		void set_springs(std::vector<Vector3>* springs, std::vector<int> propspringsindex, const bool& allow_single_outlet_connection);

		/*!
		\brief Precises index of each water tables surface
		\param  surf_water_table all surfaces of water tables
		*/
		void set_water_table_order(std::vector<Surface>* surf_water_table);
		...
		*/
		void set_waypoints(std::vector<Vector3>* waypoints);
		...
		*/
		void set_previous_networks(std::vector<Line>* previous_networks);
		...
		*/
		void set_surfaces_to_densify(std::string network_name, std::vector<Surface>* surfaces_to_densify, const int refine_surface_sampling, const bool create_surface_sampling);
		...
		*/
		void set_inception_horizons_parameters(std::vector<Surface>* inception_horizons, double inception_horizon_constraint_weight);
		...
		*/
		void set_fracture_constraint_parameters(std::vector<double>* fracture_families_orientations, std::vector<double>* fracture_families_tolerance, const double fracture_constraint_weight);
		...
		*/
		void set_no_karst_spheres_parameters(std::vector<Vector3>* sphere_centers, std::vector<double> sphere_radius);
		...
		*/
		void set_simulation_parameters(int nghb_count, const bool use_max_nghb_radius, double nghb_radius, double poisson_radius, double gamma, const bool multiply_costs);
		...
		*/
		void read_connectivity_matrix(std::vector<Vector3>* sinks, std::vector<Vector3>* springs);
		...
		*/
		...
		void run_simulation(const bool create_nghb_graph, const bool create_nghb_graph_property, const bool use_amplification_phase_vadose, const bool use_amplification_phase_phreatic, const bool use_sampling_points_,
			const double fraction_karst_perm, const double fraction_old_karst_perm, const double max_inception_surface_distance, std::vector<Vector3>* sampling_points_, const bool create_vset_sampling_, const bool use_density_property_,
			const int k_pts, std::vector<double> propdensity, std::vector<double> propikp);
	...
	private:
		...
		std::vector<Vector3> nodes_on_densify_surface;
